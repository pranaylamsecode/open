...
$query = $this->db->query("SELECT UNIX_TIMESTAMP(created_at) as date, quiz_id, score as value FROM quiz_report WHERE student_id = $student_id2 AND quiz_id = $exam_id");
...
<?php endif; ?>
<?php if(!empty($student_id) && empty($student_id2)){ ?>
...
<script>
    ...
    var comparisonData = dataset2.map(function (item) {
        return {
            date: item.date,
            comparisonValue: item.value + 10 // Adjust as needed
        };
    });
